...
00:00–00:20. This demo extends the accepted control map into a deployed EC2 environment. The native desktop client connects through local port 5599 to the EC2 Gateway, and the local Gateway listener is off. The EC2 host holds the workspace. The installed Kiro CLI backend still needs successful authentication and a verified native session in this candidate. Direct MCP calls on EC2 already demonstrate an allowed S3 read, an MCP grant denial and an AWS IAM denial. Sources: evidence/aws/stack-state.json; evidence/aws/mcp-live-receipt.json; evidence/aws/client-runtime-verification.json. Runtime status must be updated from fresh receipts before presenting the native walkthrough as completed.

## Slide 2

### Slide text

One endpoint-control box
...
The MCP process holds AWS authority. Crew reaches it through bounded tool calls.
