...
<div class="speaker-notes" hidden>{notes(note)}</div></section>'''
    slides.append({"number": number, "title": re.sub('<[^>]+>', '', title), "section": section, "html": markup, "note": note, "body": body, "footer": footer})
...
<div class="hero-bottom"><p><strong>ARM deployment, direct MCP/AWS checks and telemetry verified.</strong><br>Native Kiro CLI sign-in and session pending.</p><span class="date">Recorded September 13, 2026</span></div></div>''', old_notes[1],
...
<div class="split wide-right"><div class="stack"><p class="lead">The execution host contains the Gateway, selected backend and workspace.</p><div class="callout"><strong>Remote mode</strong>EC2 is the execution host. The laptop remains the client.</div><p class="note">The reference also includes enterprise controls and Crew capabilities beyond the verified demo.</p></div><div class="image-and-caption">{picture('evidence/browser/endpoint-slide.png', 'Accepted endpoint-control reference. One enclosure preserved. Broader capabilities require their own verification.', 'endpoint')}</div></div>''', old_notes[2],
...
<div class="table-wrap"><table><thead><tr><th>Identity</th><th>Responsibility</th><th>Authority and observed boundary</th></tr></thead><tbody>
<tr><td><code>crew</code></td><td>Gateway, backend and workspace</td><td>IPv4 metadata TCP denied<small>errno 113. No HTTP request or credential read.</small></td></tr>
<tr><td><code>mcp-demo</code></td><td>Fixed MCP tools and S3 reads</td><td>Instance role through IMDSv2<small>Two demo S3 prefixes plus SSM core permissions.</small></td></tr>
<tr><td>Administrator</td><td>Installs code and manages services</td><td>Trusted host administration<small>Core Gateway and MCP runtime files are root-owned.</small></td></tr>
</tbody></table></div><div class="callout"><strong>Crew can edit its configuration.</strong>Immutable policy and tamper resistance remain unverified.</div>''', old_notes[4],
...
subtitle="Custom App Kit page through the authenticated Gateway proxy. Backend on loopback :9102.")
...
<div class="callout"><strong>Per receipt: Gateway PID 4991, started 14:26:10 UTC, unchanged.</strong>Gateway and MCP stayed active. Active model-session continuity remains unverified.</div>''', old_notes[10],
...
copy_parts += ["## What changed", "Reorganized the accepted deck around requirements, architecture, authorization, observability and operational decisions. Added explicit assumptions and growth triggers. Kept the original evidence images, exact tool names, recorded times and native-authentication limits."]
copy_text = "\n\n".join(copy_parts) + "\n"
...
<body><header class="shell-header"><span class="brand">KiroCrew</span><div class="header-right"><span class="edition">System design / September 2026</span><a href="{download_pptx}" download="kirocrew-arm-observability-demo.pptx">Original PPTX</a></div></header>
...
manifest = {"schema": 1, "edition": "Standalone system-design presentation", "slide_count": len(slides),
            ...
            "native_kiro_cli_proof": "pending in source receipts", "prior_council_scope": "Frozen PPTX candidate-v2 only; this HTML edition has no new external council verdict",
