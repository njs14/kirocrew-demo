...
ARM deployment, direct MCP/AWS checks and telemetry verified.
...
The execution host contains the Gateway, selected backend and workspace.

Remote mode EC2 is the execution host. The laptop remains the client.

The reference also includes enterprise controls and Crew capabilities beyond the verified demo.
...
Identity / Responsibility / Authority and observed boundary /

crew / Gateway, backend and workspace / IPv4 metadata TCP denied errno 113. No HTTP request or credential read. /

mcp-demo / Fixed MCP tools and S3 reads / Instance role through IMDSv2 Two demo S3 prefixes plus SSM core permissions. /

Administrator / Installs code and manages services / Trusted host administration Core Gateway and MCP runtime files are root-owned. /

Crew can edit its configuration. Immutable policy and tamper resistance remain unverified.
...
Native model-session metrics remain pending.
...
The stopped x86 disk adds $1.92/month. Stop ARM between demos when idle.
...
04 / NATIVE ACCEPTANCE Complete CLI sign-in

Then run four native turns. crew_denied must stop automatically before MCP dispatch.
