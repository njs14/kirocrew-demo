...
catalog={'title':'KiroCrew endpoint and governance control map','baseline':'0.7.0.dev20260911060948','commit':rev,'baselineNote':'Verified nightly retained. The current nightly feed could not be refreshed during this edit.','layers':layers}
...
security=base64.b64encode((base/'kirocrew-security-layers.html').read_bytes()).decode()
aws=base64.b64encode((base/'kirocrew-aws-deployment.html').read_bytes()).decode()
...
<div class="note"><strong>Nightly baseline:</strong> 0.7.0.dev20260911060948 · fa0d8cf3c241. The current feed could not be refreshed, so this view retains the verified build. Exact rule counts and effective settings should come from the demo host’s Settings → Security and policy diagnostics.</div>
...
<h3>Slide-to-demo sequence</h3><p class="intro">1. Locate the execution host. 2. Explain L0–L5 and the policy ceiling. 3. Prove a harmless allow, approval and deny with the actual backend. 4. Show protected policy and project-skill trust. 5. Compare Crew denial, MCP grant denial and target IAM denial. 6. Correlate the decision with SEL and external evidence. Use synthetic secrets for redaction demonstrations.</p>
<footer>Source basis: pinned nightly wheel implementation, including security_posture.py, hooks.py, sandbox.py, governance.py, validation.py, sel.py and skill_trust.py; <a href="https://github.com/kirodotdev/KiroCrew/blob/main/docs/architecture/security-deep-dive.md" target="_blank" rel="noopener">current security architecture</a> supplied the six-layer naming. Both embedded diagrams passed Archify’s nine artifact checks. Static previews were reviewed. Browser interaction/containment and cloud deployment are unverified. AWS service icons remain embedded in the deployment view.</footer>
</section></main><script>
...
function showView(id){if(!['security','aws','reference'].includes(id))id='security';document.querySelectorAll('.panel').forEach(p=>p.hidden=p.id!==id);document.querySelectorAll('[data-view]').forEach(b=>b.setAttribute('aria-selected',String(b.dataset.view===id)));loadView(id);history.replaceState(null,'','#'+id)}
...
notes += ['## AWS deployment and MicroVM alternative','',
 ...
 '## Original AWS walkthrough','', (base/'kirocrew-aws-deployment-notes.md').read_text()]
