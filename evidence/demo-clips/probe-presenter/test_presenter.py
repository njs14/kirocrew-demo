"""Focused operator-boundary checks. All command runs use local test stubs."""
import copy
import http.client
...
FIXTURE = json.loads((ROOT / "evidence/aws/arm-20260913/final-direct-mcp-rehearsal.json").read_text())
...
class ParserTests(unittest.TestCase):
    ...
    def test_synthetic_accepted_steps(self):
        value = json.loads((ROOT / "evidence/live/20260913T032933Z-4efa70/receipt.json").read_text())
...
class SubprocessTests(unittest.TestCase):
    def test_fixed_command_is_exact(self):
        self.assertEqual(presenter.COMMANDS["direct"][:3], ["/usr/bin/ssh", "-T", "kirocrew-demo-admin"])
        self.assertEqual(presenter.COMMANDS["direct"][3], "sudo -u mcp-demo env DEMO_MCP_TOKEN_FILE=/etc/kirocrew-demo/mcp-token /opt/kirocrew-demo/mcp-venv/bin/python /opt/kirocrew-demo/mcp-enforcement/probe.py --expected-allowed-sha256 " + presenter.EXPECTED_SHA)
