...
def prepare(args: argparse.Namespace) -> None:
    ...
    slides = [Path(args.slides).resolve(strict=True) / f"slide-{i}.png" for i in range(1, 13)]
    ...
    with zipfile.ZipFile(deck) as z:
        ...
        if len([n for n in z.namelist() if re.fullmatch(r"ppt/slides/slide\d+\.xml", n)]) != 12:
            raise ValueError("the council packet must contain exactly 12 slides")
    for path in slides:
...
def verify_packet(packet: Path, expected: str) -> dict:
    ...
    expected_names = {"manifest.json", "deck.pptx", "notes.md", "evidence-summary.md"} | {f"slide-{i}.png" for i in range(1, 13)}
    if {p.name for p in packet.iterdir()} != expected_names:
        raise ValueError("packet must contain only deck, 12 PNGs, notes, evidence summary, manifest")
    entries = manifest.get("files", [])
    if len(entries) != 15 or {e["name"] for e in entries} != expected_names - {"manifest.json"}:
...
slide-8.png, slide-9.png, slide-10.png, slide-11.png and slide-12.png as images. The PNGs were rendered from deck.pptx;
...
This edition covers the ARM EC2 replacement and observability. Assess whether the
architecture, instance identity, capacity, CPU-credit mode, cost and retained x86
resources agree with the supplied evidence. Distinguish telemetry configuration,
actual emitted events, ingestion/retention, screenshots and queries, and a verified
security audit chain. A log reaching CloudWatch is not proof of universal capture
or tamper resistance. Check timestamps and do not merge evidence from different
hosts, versions or probes into one claimed end-to-end run. Judge the presentation
of these limits rather than asking for unrequested enterprise scope.
...
def analyze_stream(path: Path, reviewer: str, packet: Path) -> dict:
        ...
        for block in blocks:
            ...
            if kind == "tool_use" and block.get("name") in ("Read", "read_file"):
                ...
                if isinstance(file, str):
                    ...
                    if requested.parent == packet and re.fullmatch(r"slide-(?:[1-9]|1[0-2])\.png", requested.name):
    ...
    reads = [{"file": f"slide-{i}.png", "read_requested": f"slide-{i}.png" in requests.values(),
              ...
              "exact_packet_png_payload_observed": (packet / f"slide-{i}.png").is_file() and digest(packet / f"slide-{i}.png") in payloads.get(f"slide-{i}.png", set())} for i in range(1, 13)]
    ...
    return {"response_model_ids": sorted(response_models), "usage_model_ids": sorted(usage_models),
            ...
            "all_twelve_image_payloads_verified": len(images) == 12, "image_reads": reads,
...
def run_reviewer(reviewer: str, packet: Path, out: Path, prompt: str, timeout: int) -> dict:
    ...
    result["review_evidence_complete"] = bool(code == 0 and not failure and result["terminal_success"]
        ...
        and result["exact_response_model_verified"] and result["all_twelve_image_payloads_verified"]
