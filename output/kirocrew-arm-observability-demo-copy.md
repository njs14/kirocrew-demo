# KiroCrew on ARM: final slide copy and speaker notes
...
Reference map. Verified: remote

client, MCP grants and S3 IAM.

Unverified: sandbox, egress

control and SEL/HMAC.
...
IPv4 metadata TCP denied for crew UID

Observed errno 113; no HTTP request
...
Crew can edit its configuration.

Immutable policy and tamper resistance are unverified.
...
direct_mcp_service_probe, 2026-09-13 15:01:37 UTC
...
Request ID BA5BT7K9765938BT

Unauthenticated request: 401. Discovery returned four tools.

Separate direct probe. Native Crew backend verified: false.
...
48 RECORD file checks passed across arm64 and x64 bundles

EC2 server · frozen September 12

0.7.0-nightly.20260912t060850

Unofficial Linux ARM64 repack; 24 selected modules match

Different nightly builds; behavioral equivalence is unverified.
...
14 d display window · 7 d retention · 64 MiB target

Local export: 10 s · no OTLP · usage reporting off

Gateway metrics present. Native model-session metrics remain pending.
...
Collect 60 s · Refresh 15 s · Stale 180 s · 240 samples/source · App :9102

EDT (UTC−4). Client CPU: process family; server: whole host. Methods differ.
...
Stopped sample: 14:44:42 UTC · Recovery sample: 14:49:34 UTC

Screenshot clocks: EDT (UTC−4)

Per receipt: Gateway PID 4991, started 14:26:10 UTC, unchanged.

Gateway/MCP stayed active. Active model-session continuity remains unverified.
...
Compute + IPv4. ARM 40 GiB storage adds $3.20/month.

Stopped t3a.small: 24 GiB storage still costs $1.92/month.

About $107 per month if always on

Includes both disks; excludes S3, transfer, model use and tax.

Standard CPU credits: no surplus charges.

Sustained load can throttle to baseline.

Stop ARM after the demo. EBS remains billable.

Keep x86 stopped for rollback. Refresh the ARM address after restart.
