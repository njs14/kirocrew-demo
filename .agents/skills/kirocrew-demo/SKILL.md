...
Read `evidence/native-client-demo/current-status.json` and [docs/MANAGED-DEMO.md](../../../docs/MANAGED-DEMO.md) before changing the managed demo. Its recorded policy, native MCP and host observations are under `evidence/enterprise-managed/20260913/`. `HANDOFF.md` records earlier work and may be stale. Historical screenshots, recordings and receipts prove only their recorded deployment and time.
...
Complete the selected workflow with its observable result and material remaining prerequisites. Report a synthetic rehearsal, a native UI observation, a backend log and an AWS denial at their actual evidence level. A screenshot of a result is not footage of the action executing.
